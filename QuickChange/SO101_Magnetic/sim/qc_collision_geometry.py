    ...
    for signal in SIGNALS:
        body = ET.SubElement(
            ...
            "body",
            {
                ...
                "pos": f"-0.031 {SIGNAL_Y_M[signal]:.7f} 0.00875",
            },
        )
        ET.SubElement(
            body,
...
def _add_target_with_bore(body: ET.Element, tool: str, side: str, y_value: float) -> list[str]:
    ...
    for suffix, x, y, sx, sy in (
        ("xneg", -0.0045, y_value, 0.0015, 0.0060),
        ("xpos", 0.0045, y_value, 0.0015, 0.0060),
        ("yneg", 0.0, y_value - 0.0045, 0.0030, 0.0015),
        ("ypos", 0.0, y_value + 0.0045, 0.0030, 0.0015),
    ):
...
def add_tool_quick_change_interface(body: ET.Element, tool: str) -> list[str]:
    ...
    names: list[str] = []
    plate_name = f"matcha_col_{tool}_plate_core__mating_land__dock_stop_land"
    _geom(
        body,
        name=plate_name,
        geom_type="box",
        pos=(0.0, 0.0, 0.00475),
        size=(0.028, 0.025, 0.00475),
    ...
    for side, y_value in (("neg", -0.016), ("pos", 0.016)):
...
def add_supported_dock(
    ...
    *,
    ...
    support = ET.SubElement(
        worldbody,
        "body",
        {"name": f"dock_{tool}_support", "pos": f"{x_value:.9g} {y_value:.9g} 0"},
    )
    _geom(
        support,
    ...
    for side, x_rail in (("left", -0.031), ("right", 0.031)):
        _geom(
            dock,
            name=f"dock_{tool}_rail_{side}_collision",
            geom_type="box",
            pos=(x_rail, 0.0, 0.010),
            size=(0.003, 0.030, 0.010),
            rgba=rgba,
        )
    _geom(
        dock,
