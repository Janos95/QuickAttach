...
`box_stack_scene.xml` adds a work surface, two dynamic 52 mm cubes, a stack
target, and a fixed overview camera to the calibrated SO-101 model.
`box_stack_demo.py` runs a small scripted task using damped least-squares
position IK. For this first prototype, grasping is assisted by an equality weld
that activates only after the jaw closes near a cube. Cube/table/stack contacts
remain physical. This makes the task deterministic while leaving the end
effector geometry easy to replace later.
...
.venv/bin/python Simulation/SO101/box_stack_demo.py
```

For CI or machines without a display:

```bash
.venv/bin/python Simulation/SO101/box_stack_demo.py --headless
```

The process exits with status 0 when both cubes finish inside the target stack
tolerance. Use `--save-preview Simulation/SO101/box_stack_preview.png` with an
off-screen OpenGL backend such as `MUJOCO_GL=egl` to capture the final frame.
